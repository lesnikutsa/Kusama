...
update() {
	...
	if ! grep -q "Image is up to date for" <<< "$status"; then
		...
		docker run -dit --name kusama_lesnik --restart always --network host -v $HOME/.kusama:/data -u $(id -u ${USER}):$(id -g ${USER}) parity/polkadot:latest --base-path /data --chain kusama --validator --name "lesnik_utsa2" --state-pruning 16 --blocks-pruning 16 --insecure-validator-i-know-what-i-do --public-addr /ip4/$(wget -qO- eth0.me)/tcp/30333 --port 30333 --rpc-port 9933 --prometheus-port 9615 --in-peers 12 --out-peers 8 --telemetry-url 'wss://telemetry.polkadot.io/submit/ 1' --telemetry-url 'wss://telemetry-backend.w3f.community/submit 1'
	else
